...
return idb.open('restaurants-reviews',1).then(function(db){var tx=db.transaction(storeName,'readonly');var store=tx.objectStore(storeName);return store.getAll();}).then((rs)=>{console.log("All Data From IndexDB",rs);if(!rs.length){return fetch(event.request.url).then((response)=>{return response.json().then(function(data){addAllToDB(storeName,data);console.log('Saving to DB and responding from FETCH',data);var init={status:200,statusText:"OK",headers:{'Content-Type':'application/jso'}};const fetchResponse=new Response(JSON.stringify(data),init);return fetchResponse;})})}else{console.log('Responding from IndexDB');var init={status:200,statusText:"OK",headers:{'Content-Type':'application/jso'}};const indexDBResponse=new Response(JSON.stringify(rs),init);console.log("Response from indexDB to send to fetch ",rs);return indexDBResponse;}})}else{const cachedResponse=await caches.match(event.request);if(cachedResponse){console.log('Found ',event.request.url,' in cache');return cachedResponse;}
return fetch(event.request).then(function(cachedResponse){return caches.open(staticCacheName).then(function(cache){if(event.request.url.indexOf('maps')<0){console.log('Saving '+event.request.url+' into cache.');cache.put(event.request.url,cachedResponse.clone());}
